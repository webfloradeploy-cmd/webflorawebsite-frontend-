    ...
    try {
      ...
      if (blog) {
        setFormData({
          ...
          metaExtraHead: blog.metaExtraHead || "",
          datePosted: blog.datePosted ? new Date(blog.datePosted).toISOString().split('T')[0] : "",
          imageUrl: blog.image || ""
        });
        if (blog.image) {
          setPreviewImage(blog.image.startsWith('http') ? blog.image : `${API_BASE_URL}/${blog.image}`);
        }
      }
    } catch (err) {
        ...
        <header className="flex justify-between items-center mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-400 text-sm">Update and manage your blog content.</p>
            </div>
          </div>
